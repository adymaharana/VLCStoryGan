...
def make_shifted_mask(input_mask, max_v_len, max_t_len, memory_len=0):
    ...
    if len(input_mask.shape) == 2:
        ...
        bsz, seq_len, _ = input_mask.shape
    else:
        raise ValueError
...
def make_pad_shifted_mask(input_mask, max_v_len, max_t_len, memory_len=0):
    ...
    if len(input_mask.shape) == 2:
        ...
        # pad_shifted_mask = shifted_mask * input_mask
        pad_shifted_mask = input_mask
    else:
        raise ValueError
...
def make_video_only_mask(input_mask, max_v_len):
    if len(input_mask.shape) == 2:
        ...
        # for MARTT, we want the memory initializer to consider all words, the last layer in mask is all ones.
        video_only_mask = copy.deepcopy(torch.diagonal(input_mask, dim1=-2, dim2=-1))
        video_only_mask[:, max_v_len:] = 0
    else:
        raise ValueError
...
class BertLayerWithMemory(nn.Module):
    ...
    def forward(self, prev_m, hidden_states, attention_mask):
        ...
        if len(attention_mask.shape) == 2:
            ...
            # For MARTT, adjust matrix for (N, L, l) --> (N, L, M+L)
            raw_memory_attention_mask = torch.cat(
                [attention_mask.new_ones(bsz, max_t_len, n_memory_cells), attention_mask], -1)  # (N, L, M+L)
        else:
            raise ValueError
...
class BertEncoderWithMemoryForTree(nn.Module):
    def __init__(self, config):
        super(BertEncoderWithMemoryForTree, self).__init__()
        self.layer = nn.ModuleList([BertLayerWithMemory(config) for _ in range(config.num_hidden_layers)])
        print("Initialized")
    ...
    def forward(self, prev_ms, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        """
        all_encoder_layers = []
        # print(len(prev_ms), hidden_states.shape, attention_mask.shape)
        for layer_idx, layer_module in enumerate(self.layer):
            prev_ms[layer_idx], hidden_states = layer_module(prev_ms[layer_idx], hidden_states, attention_mask[:, layer_idx, :, :])
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
...
class MemoryUpdater(nn.Module):
    ...
    def forward(self, prev_m, input_states, attention_mask):
        ...
        if len(attention_mask.shape) == 2:
            ...
            # for MARTT memory updation, use all ones from memory mask
            update_mask = torch.diagonal(attention_mask, dim1=-2, dim2=-1).unsqueeze(1).repeat(1, n_memory_cells, 1)  # (N, M, L)
        else:
            raise ValueError
