...
class GANTrainer(object):
    ...
    def load_network_stageI(self):
        if self.cfg.RECURRENT:
            from recurrent_model import StoryGAN, STAGE1_D_IMG, STAGE1_D_STY_V2, StoryMartGAN
        else:
    ...
    def load_dual_model(self):
        ...
        if self.cfg.DENSECAP_DUAL:
            ...
            checkpoint = torch.load('../../densecap-pytorch/model_params/train_all_val_all_bz_2_epoch_10_inject_init.pth.tar')
    ...
    def train(self, imageloader, storyloader, testloader, stage=1):
        ...
        if self.cfg.IMG_DUAL or self.cfg.STORY_DUAL or self.cfg.DENSECAP_DUAL:
                ...
                if self.use_image_disc:
                    ...
                    im_errD, imgD_loss_report = \
                        compute_discriminator_loss(netD_im, im_real_imgs, im_fake,
                                                   ...
                                                   im_mu, self.gpus, mode='image', dual=self.img_dual,
                                                   real_captions=None if not self.img_dual else (im_input_ids, im_masks),
                                                   contrastive=self.cfg.IMG_CONTRAST, real_caption_embeds=im_motion_input if self.cfg.IMG_CONTRAST else None)
                    im_errD.backward()
                    im_optimizerD.step()
                else:
                ...
                if self.use_story_disc:
                    ...
                    st_errD, stD_loss_report = \
                        compute_discriminator_loss(netD_st, st_real_imgs, st_fake,
                                                   ...
                                                   st_mu, self.gpus, mode='story', dual=self.story_dual,
                                                   real_captions=None if not self.story_dual else (st_input_ids, st_masks),
                                                   contrastive=self.cfg.STORY_CONTRAST, real_caption_embeds=st_motion_input if self.cfg.STORY_CONTRAST else None)
                    st_errD.backward()
                    st_optimizerD.step()
                else:
                ...
                for g_iter in range(self.cfg.TRAIN.UPDATE_RATIO):
                    ...
                    if self.use_image_disc:
                        im_errG, imG_loss_report = compute_generator_loss(netD_im, im_fake,
                                                               ...
                                                               mode='image', dual=self.img_dual,
                                                               real_captions=None if not self.img_dual else (im_input_ids, im_masks),
                                                                          contrastive=self.cfg.IMG_CONTRAST, fake_caption_embeds=im_motion_input if self.cfg.IMG_CONTRAST else None)
                        if self.cfg.DENSECAP_DUAL:
                            if self.cfg.CUDA:
                                ...
                                targets = {'boxes': im_batch['boxes'], 'caps': im_batch['caps'], 'caps_len': im_batch['caps_len']}
                            im_errDual, im_densecap_loss_report = compute_dual_densecap_loss(netDual, im_fake, targets, self.gpus, self.cfg, mode='image')
                    ...
                    if self.use_story_disc:
                        st_errG, stG_loss_report = compute_generator_loss(netD_st, st_fake,
                                                            ...
                                                            mode='story', dual=self.story_dual,
                                                            real_captions=None if not self.story_dual else (st_input_ids, st_masks),
                                                                          contrastive=self.cfg.STORY_CONTRAST, fake_caption_embeds=st_motion_input if self.cfg.STORY_CONTRAST else None)
                        if self.cfg.DENSECAP_DUAL:
                            if self.cfg.CUDA:
                                ...
                                targets = {'boxes': st_batch['boxes'], 'caps': st_batch['caps'], 'caps_len': st_batch['caps_len']}
                            st_errDual,  st_densecap_loss_report = compute_dual_densecap_loss(netDual, st_fake, targets, self.gpus, self.cfg, mode='story')
                    else:
    ...
    def train_ssl(self, imageloader, ssl_imageloader, storyloader, ssl_storyloader, testloader, stage=1):
        self.imageloader = imageloader
        self.imagedataset = None
        if stage == 1:
            netG, netD_im, netD_st = self.load_network_stageI()
                ...
                if self.use_image_disc:
                    ...
                    im_errD, imgD_loss_report = \
                        compute_discriminator_loss(netD_im, im_real_imgs, im_fake,
                                                   im_real_labels, im_fake_labels, im_labels,
                                                   im_mu, self.gpus, mode='image', dual=self.img_dual,
                                                   real_captions=None if not self.img_dual else (
                                                   im_input_ids, im_masks))
                    im_errD.backward()
                    im_optimizerD.step()
                else:
                    im_errD = torch.tensor(0)
                ...
                if self.use_story_disc:
                    ...
                    st_errD, stD_loss_report = \
                        compute_discriminator_loss(netD_st, st_real_imgs, st_fake,
                                                   st_real_labels, st_fake_labels, st_labels,
                                                   st_mu, self.gpus, mode='story', dual=self.story_dual,
                                                   real_captions=None if not self.story_dual else (
                                                   st_input_ids, st_masks))
                    st_errD.backward()
                    st_optimizerD.step()
                else:
                    st_errD = torch.tensor(0)
