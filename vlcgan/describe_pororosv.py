...
class VideoFolderDataset(torch.utils.data.Dataset):
    def __init__(self, folder, min_len=4):
        self.dir_path = folder
        self.images = []
        images = np.load(folder + 'img_cache' + str(min_len) + '.npy', encoding='latin1')
        followings = np.load(folder + 'following_cache' + str(min_len) + '.npy')
        for list in tqdm(followings):
            for path in list:
                if path not in images:
                    self.images.append(path)
        print("Total number of clips {}".format(len(self.images)))

    def get_images(self, im):
        shorter, longer = min(im.size[0], im.size[1]), max(im.size[0], im.size[1])
        video_len = int(longer/shorter)
        return [im.crop((0, se * shorter, shorter, (se+1)*shorter)) for se in range(video_len)]

    def __getitem__(self, item):
        v = self.images[item]
        img_id = str(v).replace('.png', '')[3:-2]
        path = self.dir_path + img_id + '.png'
        images = self.get_images(Image.open(path))
        return [path.replace(self.dir_path, '') + '_' + str(i) for i in range(len(images))], images
...
def img_to_tensor(img_list):
    ...
    for img in img_list:
        if img.size == (640, 480):
            ...
            img_tensors.append(transforms.ToTensor()(transforms.Resize((480, 640))(img)))
    return img_tensors


def describe_images(model, img_folder, device, console_args):

    assert isinstance(console_args.batch_size, int) and console_args.batch_size > 0
    img_dataset = VideoFolderDataset(img_folder)
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(0, len(img_dataset), console_args.batch_size)):
            img_paths, imgs = img_dataset[i:i+console_args.batch_size]
            for j in range(0, len(imgs), 5):
                image_tensors = img_to_tensor(imgs[j:j+5])
    ...
    return all_results, all_img_paths
...
def save_results_to_file(img_list, all_results, console_args):
    ...
    with open(os.path.join(console_args.result_dir, 'result_extra.json'), 'w') as f:
...
def main(console_args):
    ...
    # === inference ====
    all_results, img_list = describe_images(model, console_args.img_path, device, console_args)
